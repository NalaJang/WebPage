...
public class HotelQnADao {
	...
	public void insertqna(HotelQnAVo HotelQnAVo, String session_email) throws Exception {
		...
		try {
			pstmt = conn.prepareStatement("INSERT INTO QNA (title, subject, content, email) VALUES(?,?,?,?)");
			pstmt.setString(1, HotelQnAVo.getSubject());
			pstmt.setString(2, HotelQnAVo.getTitle());
			pstmt.setString(3, HotelQnAVo.getContent());
			pstmt.setString(4, session_email);
			pstmt.executeUpdate();
		} catch (SQLException e) {
			throw e;
		} finally {
	...
	//						답변 작성
	//----------------------------------------------------------------
	public int update(String reply, int qseq) throws Exception {
		conn = ds.getConnection();
	...
	//						답변 목록
	//----------------------------------------------------------------
	public ArrayList<HotelQnAVo> qnaReplyList(HotelQnAVo qna) throws Exception {
		
		ArrayList<HotelQnAVo> qnaList = new ArrayList<>();
		conn = ds.getConnection();
		try {

			pstmt = conn.prepareStatement("SELECT * FROM QNA WHERE QSEQ = ? ORDER BY QSEQ DESC");
			pstmt.setInt(1, qna.getQseq());
			rs = pstmt.executeQuery();
