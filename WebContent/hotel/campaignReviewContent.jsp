...
<form name = "frm" action = "campaignReplyInsert.do" method = "post">
...
<div class = "container">
	...
	<a href = "campaignReviewList.do">활동후기 > </a>
		...
		<c:choose>
			<c:when test = "${hotelMember.email != null}">
				<c:if test = "${reviewVo.email eq hotelMember.email}">
					...
					<a href = "campaignReviewDelete.do?qseq=${reviewVo.qseq}">[삭제]</a>
					
					<a href = "campaignReviewEditForm.do?qseq=${reviewVo.qseq}">[수정]</a>
				</c:if>
			</c:when>
					...
					<c:choose>
						...
						<c:when test = "${hotelMember.email != null}">
							<c:if test = "${replyList.email eq hotelMember.email}">
								...
								<a href = "campaignReplyDelete.do?replynum=${replyList.replynum}&qseq=${replyList.qseq}" onclick = "alert('삭제되었습니다.')">[삭제]</a>
							</c:if>
							
						</c:when>
						
						<c:when test = "${hotelWorker.email != null}">
							<a href = "campaignAdmin_delete.do?replynum=${replyList.replynum}&qseq=${replyList.qseq}">[삭제]</a>
						</c:when>
